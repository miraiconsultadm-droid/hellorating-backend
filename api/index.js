  ...
  try {
    ...
    if (supabase) {
      ...
      const mappedData = {
        id: data.id,
        name: data.nome,
        mainMetric: data.metrica,
        redirectEnabled: data.redirecionar_google,
        redirectRule: data.regra_redirecionamento,
        feedbackEnabled: data.habilitar_feedback,
        feedbackText: data.texto_feedback,
...
app.post('/api/campaigns', async (req, res) => {
  ...
  try {
    ...
    const { name, mainMetric, redirectEnabled, redirectRule, feedbackEnabled, feedbackText, questions, googlePlaceId } = req.body;
    
    const campaignToInsert = {
      nome: name,
      metrica: mainMetric,
      redirecionar_google: redirectEnabled,
      regra_redirecionamento: redirectRule,
      habilitar_feedback: feedbackEnabled,
      texto_feedback: feedbackText,
      perguntas: questions || [],
      // Adicionar user_id e token, se necessário e disponível
      // user_id: req.user.id, // Exemplo, se houver autenticação
...
app.get('/api/surveys/:id', async (req, res) => {
  ...
  try {
    ...
    res.json({ campaign: mappedData, questions: mappedData.questions });
